...
const Footer = ({ tokenAddress, contractAddress: mainAddress, projectId = 'xen' }) => {
  ...
  const maybeContractAddress =
    mainAddress && mainAddress.toString().startsWith('0x') ? mainAddress : '';

  const maybeTokenAddress =
    tokenAddress && tokenAddress.toString().startsWith('0x') ? tokenAddress : '';

  const mainBlockExplorerUrl =
    typeof maybeContractAddress === 'string' &&
    currentNetwork?.explorerUrl &&
    `${currentNetwork.explorerUrl}address/${maybeContractAddress}`;

  const tokenBlockExplorerUrl =
    typeof maybeTokenAddress === 'string' &&
    currentNetwork?.explorerUrl &&
    `${currentNetwork.explorerUrl}address/${maybeTokenAddress}`;

  const contractAddressString =
    projectId === 'xen' || projectId === 'xenft'
      ? `XEN: ${((maybeContractAddress || '').toString() || '-').slice(0, 12)}`
      : `${project.name}: ${((maybeContractAddress || '').toString() || '-').slice(0, 12)}`;

  const tokenAddressString = `${project.tokenSymbol || ''} token: ${(
    (maybeTokenAddress || '').toString() || '-'
  ).slice(0, 12)}`;
        ...
        <Grid item xs={12} md={6}>
          ...
          >
            {maybeContractAddress && (
              <Tooltip title={`Explore ${project.name} Contract and Transactions`}>
                <StyledAnchor
                  href={mainBlockExplorerUrl as string}
                  target="_blank"
                  rel="noreferrer"
                  style={{
                    display: 'unset'
            ...
            {maybeTokenAddress && (
              <Tooltip title={`Explore ${project.tokenSymbol} Token and Transactions`}>
                <StyledAnchor
                  href={tokenBlockExplorerUrl as string}
                  target="_blank"
                  rel="noreferrer"
                  style={{
                    display: 'unset'
